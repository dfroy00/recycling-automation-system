...
> ⚠️ **計費引擎是整個系統最高風險的核心模組**，涉及多種組合（品項方向 × 車趟費模式 × 附加費用頻率 × 開票方式），必須有完整的單元測試覆蓋。

以下為必須覆蓋的測試組合矩陣：

| # | 測試案例 | 品項方向 | 車趟費 | 附加費用 | 開票方式 | 預期驗證 |
|---|---------|---------|--------|---------|---------|---------|
| 1 | 純應收客戶 | 全 receivable | 按次 | 無 | 淨額 | 應付=0，淨額=應收合計 |
| 2 | 純應付客戶 | 全 payable | 無 | 無 | 淨額 | 應收=0，淨額為負 |
| 3 | 混合方向客戶 | receivable + payable | 按月 | monthly（應收+應付各一） | 淨額 | 正確分組加總 |
| 4 | 含免費品項 | receivable + free | 按次 | 無 | 淨額 | free 品項不計入金額 |
| 5 | 分開開票 | receivable + payable | 按次 | per_trip（應收） | 分開 | receivable_*/payable_* 各自正確 |
| 6 | 無車趟月份 | - | 按次 | monthly（應收） | 淨額 | 品項=0，車趟費=0，僅附加費用 |
| 7 | 按趟附加費用 | receivable | 無 | per_trip × 3 趟 | 淨額 | per_trip 費用 = 金額 × 當月趟數 |
| 8 | 按月車趟費 | payable | 按月(3000) | 無 | 淨額 | 車趟費=3000（不論趟數） |
| 9 | 稅額四捨五入 | receivable | 無 | 無 | 淨額 | 稅額=Math.round(小計×0.05) |
| 10 | 大量品項（50+）| 混合 | 按次 | monthly+per_trip | 分開 | 正確加總，無精度問題 |
| 11 | Decimal 精度 | receivable | 按次 | 無 | 淨額 | 0.1+0.2 等浮點數問題不影響計算 |
| 12 | 按趟結算明細 | receivable + payable | 按次 | per_trip | 淨額 | 單趟計算正確，附加費用按趟 |
...
除了每月 5 號的自動排程，系統**必須支援隨時手動觸發月結產出**，以應對以下實務場景：

| 場景 | 處理方式 |
|------|---------|
| 資料補齊延遲（5 號前未補完） | 手動補完後，在「月結管理」頁面點擊「重新產出」 |
| 自動產出有部分客戶失敗 | 修正資料後，手動觸發僅產出失敗的客戶 |
| 臨時需要提前產出 | 手動觸發，不受 5 號排程限制 |
| 需重新計算（發現品項/車趟有誤） | 刪除草稿 → 修正資料 → 手動重新產出 |

**手動觸發 API：**
- `POST /api/statements/generate` body: `{ yearMonth: "2026-01" }` — 產出指定月份全部月結客戶
- `POST /api/statements/generate` body: `{ yearMonth: "2026-01", customerId: 5 }` — 僅產出單一客戶
...
月結明細產出是批次作業（一次處理所有月結客戶），部分客戶計算失敗不應影響其他客戶：

```
月結批次產出（每月 5 號）
│
├─ 逐一處理每個月結客戶
│   ├─ 計算成功 → 建立 Statement (draft)
│   └─ 計算失敗 → 記錄到 system_logs，繼續處理下一位客戶
│
├─ 批次完成後彙總
│   ├─ 成功：N 筆
│   ├─ 失敗：M 筆（附客戶 ID 和錯誤原因）
│   └─ 跳過：K 筆（無車趟資料的客戶）
│
└─ 若有失敗 → 自動寄送錯誤報告 Email 給管理員（ADMIN_EMAIL）
```

**失敗後處理方式：**
- 管理員收到錯誤報告後，排查問題並修正資料
- 透過 `POST /api/statements/generate` 手動觸發重新產出（僅產出尚未有明細的客戶，已產出的不會重複）
- 也可針對單一客戶手動觸發：`POST /api/statements/generate` body: `{ customerId: 5, yearMonth: "2026-01" }`

#### 同步衝突處理（手動輸入 vs 外部同步）

當同一筆車趟同時有手動輸入和外部同步資料時的處理策略：

| 場景 | 處理方式 |
|------|---------|
| 手動建立的 trip，POS 同步匹配到 | 比對 external_id → 不建立新 trip，記錄 system_log「疑似重複」，由人工確認 |
| POS 同步建立的 trip，使用者手動修改品項 | 允許修改，修改後 source 保持 pos_sync，但 trip_items 以人工修改為準 |
| 同步進行中又手動操作 | 同步 API 加入簡易鎖定機制（Redis 或 DB flag），防止同時執行 |

**同步鎖定機制（簡易版）：**
- 在 system_logs 中記錄同步開始/結束
- 同步 API 執行前檢查是否有進行中的同步任務
- 若有進行中的同步，回傳 `409 Conflict`：「同步進行中，請稍後再試」
- 同步超過 5 分鐘未完成視為逾時，自動解鎖
...
**statements 新增欄位（追蹤重試）：**

| 欄位 | 型別 | 說明 |
|------|------|------|
| send_retry_count | Int | 寄送重試次數（預設 0） |
| send_error | String? | 最後一次寄送失敗原因 |
...
所有列表 API 預設分頁，避免大量資料回傳：

```
GET /api/trips?page=1&pageSize=20&customerId=1&dateFrom=2026-01-01
...
- 所有 GET 列表端點皆支援分頁（sites、items、customers、contracts、trips、statements）
- 下拉選單用的 API（如站區列表選單）可帶 `?all=true` 取消分頁（資料量小的主檔）
...
|---------|---------|---------|
| < 100 | < 30 秒 | 同步處理即可 |
| 100 ~ 500 | 30 秒 ~ 3 分鐘 | 加入進度追蹤（system_logs 記錄進度） |
| > 500 | > 3 分鐘 | 未來考慮佇列處理（MVP 暫不需要） |

**MVP 處理方式：**
- 排程觸發為非同步（不阻塞 API 回應）
- 手動觸發 API 立即回傳 `202 Accepted`，背景處理
- 處理進度透過 `GET /api/schedule` 查詢
- 處理完成後寄送結果 Email 給管理員

#### 報表產出效能

PDF 和 Excel 產出是 CPU 密集操作：

- **PDF 產出**：單份明細 < 1 秒，可即時回傳
- **Excel 站區彙總**：含大量資料時可能 3-5 秒，加入 response timeout 設定為 30 秒
- **並發限制**：同時產出報表的請求限制為 5 個（避免記憶體溢出），超過時回傳 `503 Service Unavailable`
...
```
備份策略：
├─ 每日自動備份：每天 02:00 執行 pg_dump
├─ 備份保留：保留最近 30 天的備份
├─ 備份位置：Docker volume 映射到宿主機（/backups/）
├─ 月結前特別備份：每月 4 號額外執行一次備份（月結產出前）
└─ 備份驗證：每週自動測試還原一次（可選）
```

**docker-compose.prod.yml 備份設定：**
...
- [ ] 月結批次產出有失敗時，自動寄送錯誤報告給管理員
- [ ] 手動觸發月結產出正常運作（全部客戶 / 指定單一客戶）
- [ ] 月結產出防重複機制正常（已有明細的客戶不重複產出）
- [ ] Email 寄送失敗自動重試，最多 3 次
- [ ] 超過重試上限後標記失敗，通知管理員，不再自動重試
- [ ] 同步 API 加入鎖定機制，防止同時執行多個同步任務
- [ ] 同步衝突（手動 vs 外部同步）正確處理，不產生重複資料

### 效能
- [ ] 所有列表 API 支援分頁（預設 pageSize=20）
- [ ] 手動觸發月結產出回傳 202，背景非同步處理
- [ ] 報表產出 response timeout 設定為 30 秒
- [ ] 同時產出報表請求限制為 5 個

### 資料備份
- [ ] 生產環境每日自動備份 PostgreSQL
- [ ] 備份保留 30 天，自動清理過期備份
- [ ] 每月 4 號（月結前）額外執行一次備份
